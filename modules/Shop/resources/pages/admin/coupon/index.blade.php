...
<div class="container mx-auto px-0 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-24" x-data="{ open: false }">
  <x-shop::jet.action-section>
    ...
    <x-slot name="description">
      ...
      <div class="mt-6">
        <x-shop::shared.link :to="route('shop.coupons.create')">
          {{ __('Create {x}', ['x' => __('Coupon')]) }}
        </x-shop::shared.link>
      </div>
    </x-slot>

    <x-slot name="content">
      @unless ($coupons->count())
        <div class="">
                        ...
                        <tr>
                          ...
                          <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                              <span class="me-2 text-gray-500">{{ __('Allowed') }}:</span>
                              <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                {{ $coupon->allowed }}
                              </span>
                            </div>
                            <div class="flex items-center">
                              <span class="me-2 text-gray-500">{{ __('Used') }}:</span>
                          ...
                          <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                            <div class="flex items-stretch justify-end">
                              <a href="{{ route('shop.coupons.edit', ['coupon' => $coupon->id]) }}"
                              ...
                              {{-- @if ($confirming == $coupon->id)
                                <button type="button" wire:click="removeCoupon('{{ $coupon->id }}')"
                                  class="flex items-center justify-center px-3 py-2 border border-transparent text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus-default">
                                  {{ __('Sure?') }}
                                </button>
                              ...
                              @endif --}}
                              <button type="button" @click="open = '{{ $coupon->id }}'"
  ...
  <div x-show="open">
    <x-shop::alpine.modal :backdrop="false">
      <h2 class="font-medium text-prominent">{{ __('Delete :Model!', ['model' => __('Coupon')]) }}</h2>
      ...
      <div class="mt-6 flex justify-end gap-2">
        ...
        <button type="button" @click="() => { $wire.removeCoupon(open); open = false; }" class="x-focus btn-danger">
          {{ __('Yes, Delete') }}
        </button>
      </div>
    </x-shop::alpine.modal>
