...
class Captcha
{
    ...
    public function create(string $config = 'default', bool $api = false)
    {
        ...
        $this->canvas = $this->imageManager->createImage($this->width, $this->height)->fill($this->bgColor);

        if ($this->bgImage) {
            $this->image = $this->imageManager->decode($this->background())->resize(
                $this->width,
                $this->height
            );
        ...
        return $api ? [
            ...
            'key'       => $generator['key'],
            'img'       => $this->image->encodeUsingFileExtension('jpg')->toDataUri(),
        ] : new Response($this->image->encodeUsingFileExtension('jpg'), 200, [
