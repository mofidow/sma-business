            ...
            <tfoot v-if="typeConfig.showPrices !== false" class="break-inside-avoid divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <template v-if="(type === 'sale' || type === 'purchase') && record.return_orders && record.return_orders.length > 0">
                <tr v-for="returnOrder in record.return_orders" :key="returnOrder.id">
                  <th :colspan="colspan" class="p-2 text-end text-lg font-bold">{{ $t('Return') }}: {{ returnOrder.reference }}</th>
                  <th class="p-2 text-end text-lg font-bold whitespace-nowrap text-red-600 dark:text-red-400">
                ...
                >
                  <th :colspan="colspan" class="p-2 text-end text-lg font-bold">
                    {{ $t('Return Payment') }} ({{ returnOrder.return_payment_method }})
                  </th>
                  <th class="p-2 text-end text-lg font-bold text-red-600 dark:text-red-400">
