          ...
          const item = calculate_item(
            {
              ...
              serials_text: i.serials?.map(s => s.number).join('\n') || '',
...
async function handleBarcodeScan(code) {
  ...
  const results = await searchItems(code, 'purchase', true);
            ...
            <table class="w-full min-w-[500px] divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <template v-for="(item, index) of form.items" :key="item.code">
                <tbody v-if="item.product.has_variants == 1 && item.variations && item.variations.length">
                  ...
                  <tr v-if="item.product.has_serials == 1">
                    <td class="w-8 p-2"></td>
                    <td colspan="100%" class="p-2">
                      <Textarea
                        label=""
                        rows="3"
                        v-model="item.serials_text"
                        :placeholder="$t('Enter serial numbers, one per line')"
                        @change="
                          () => {
                            selectedItem = item;
                            updateItem();
                      ...
                      <p class="text-mute mt-1 text-xs">
                        {{
                          $t('{count} serial(s) entered, {expected} expected', {
                            count: item.serials_text ? item.serials_text.split('\n').filter(s => s.trim()).length : 0,
