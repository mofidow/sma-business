...
import { $extras, calculate_item, check_promotions, discount_keypress, convert_to_base_unit, searchItems } from '@/Core';
          ...
          const item = calculate_item(
            {
              ...
              serials: i.serials?.map(s => s.id) || [],
              available_serials: i.serials?.map(s => ({ id: s.id, number: s.number })) || [],
                ...
                <tbody v-if="item.product.has_variants == 1 && item.variations && item.variations.length">
                  ...
                  <tr>
                    ...
                    <td class="p-2 font-bold">
                      ...
                      <div v-if="item.serials?.length" class="text-mute text-xs">
  ...
  <Modal :show="openItemModal" max-width="xl" @close="openItemModal = false" :overflow="true" :backdrop="false">
    ...
    <div v-if="selectedItem">
      ...
      <div>
        ...
        <div v-if="selectedItem.product.has_serials == 1" class="col-span-full mb-6 px-6">
          <AutoComplete
            :json="true"
            value-key="id"
            :multiple="true"
            ...
            :searchable="true"
            :label="$t('Serials')"
            v-model="selectedItem.serials"
            :max="Number(selectedItem.quantity) || 1"
            @change="serials => (selectedItem.available_serials = serials)"
            :suggestions="route('search.serials') + '?product_id=' + selectedItem.product_id"
          />
        </div>
